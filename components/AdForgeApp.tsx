...
type ForgedAd = { id: string; title: string; status: 'draft'|'complete'; script_id?: string; sections?: any[]; voiceover_url?: string; voiceover_voice?: string; music_url?: string; music_name?: string; metadata?: any; created_at?: string; updated_at?: string }
...
const FALLBACK_TRACKS = [
  { id:"f1", name:"Upbeat Corporate", tags:"upbeat, corporate, motivational", duration:120, url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3", preview_url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3", artist:"SoundHelix" },
  { id:"f2", name:"Inspiring Piano", tags:"piano, inspiring, emotional", duration:180, url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3", preview_url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3", artist:"SoundHelix" },
  { id:"f3", name:"Energetic Beat", tags:"energetic, beat, fast", duration:90, url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3", preview_url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3", artist:"SoundHelix" },
  { id:"f4", name:"Calm Acoustic", tags:"calm, acoustic, relaxing", duration:150, url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3", preview_url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-4.mp3", artist:"SoundHelix" },
  { id:"f5", name:"Fun & Playful", tags:"fun, playful, light", duration:110, url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-5.mp3", preview_url:"https://www.soundhelix.com/examples/mp3/SoundHelix-Song-5.mp3", artist:"SoundHelix" },
]
...
function MultiSelect({ label, options, selected, onChange }: any) {
  ...
  return <div ref={ref} style={{ position:"relative" }}>
    ...
    {open&&<div style={{ position:"absolute",top:"calc(100% + 4px)",left:0,background:C.surface,border:"1px solid "+C.border,borderRadius:10,padding:6,zIndex:200,minWidth:170,maxHeight:220,overflowY:"auto",boxShadow:"0 8px 24px #0008" }}>
      {options.map((opt:string)=>{ const active=sel.includes(opt); return <div key={opt} onClick={()=>onChange(active?sel.filter(x=>x!==opt):[...sel,opt])} style={{ display:"flex",alignItems:"center",gap:8,padding:"7px 9px",borderRadius:7,cursor:"pointer",background:active?C.accentSoft:"transparent",color:active?C.accent:C.text,fontSize:13 }}><div style={{ width:14,height:14,borderRadius:3,border:"2px solid "+(active?C.accent:C.border),background:active?C.accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0 }}>{active&&<span style={{ color:"#fff",fontSize:8,fontWeight:900 }}>✓</span>}</div>{opt}</div> })}
...
// ── Voiceover Generator ───────────────────────────────────────────────────
function VoiceoverGenerator({ sections, savedVoiceoverUrl, onSave }: any) {
  ...
  useEffect(()=>{
    ...
    fetch("/api/elevenlabs/voices").then(r=>r.json()).then(d=>{
      if(d.voices&&d.voices.length>0){
        ...
        setError(d.error||"No voices available — check your ElevenLabs API key in Vercel settings.")
      }
  ...
  return <div style={{ background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:20 }}>
    ...
    <div style={{ fontSize:13,color:C.muted,marginBottom:16 }}>Generate a voiceover for your ad script using ElevenLabs.</div>

    {loading&&<div style={{ color:C.muted,fontSize:13,padding:"20px 0",textAlign:"center" }}>Loading voices…</div>}

    {!loading&&error&&voices.length===0&&(
      <div style={{ background:"#ef444422",border:"1px solid #ef444433",borderRadius:8,padding:"10px 12px",fontSize:12,color:"#ef4444",marginBottom:12 }}>{error}</div>
    )}
    ...
    {!loading&&voices.length>0&&<>
      <div style={{ marginBottom:12 }}>
        ...
        <div style={{ maxHeight:200,overflowY:"auto",border:"1px solid "+C.border,borderRadius:10 }}>
          {filteredVoices.map((v:any)=>(
            <div key={v.id} onClick={()=>setSelectedVoice(v.id)} style={{ display:"flex",alignItems:"center",gap:10,padding:"9px 12px",cursor:"pointer",background:selectedVoice===v.id?C.accentSoft:"transparent",borderBottom:"1px solid "+C.border }}>
              ...
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:600,fontSize:13,color:selectedVoice===v.id?C.accent:C.text }}>{v.name}</div>
                <div style={{ fontSize:10,color:C.muted }}>{[v.gender,v.age,v.accent].filter(Boolean).join(" · ")}{v.description?` · ${v.description}`:""}</div>
              </div>
      ...
      <div style={{ background:C.surface,border:"1px solid "+C.border,borderRadius:8,padding:"10px 12px",fontSize:12,color:C.muted,marginBottom:14,maxHeight:70,overflowY:"auto",lineHeight:1.6 }}>
        <strong style={{ color:C.text }}>Script:</strong> {fullScript||"No spoken words yet — add them in the script editor first."}
      </div>
      ...
      {audioUrl&&<div style={{ marginBottom:14 }}>
        <Label>Generated Voiceover {selectedVoiceObj?`— ${selectedVoiceObj.name}`:""}</Label>
      ...
      <div style={{ textAlign:"center",marginTop:10 }}><button onClick={()=>onSave(null,null)} style={{ background:"none",border:"none",color:C.muted,cursor:"pointer",fontSize:12,textDecoration:"underline" }}>Skip voiceover</button></div>
    </>}
  </div>
}

// ── Music Picker ──────────────────────────────────────────────────────────
function MusicPicker({ suggestedMood, savedMusicUrl, savedMusicName, onSave }: any) {
  ...
  async function search(){
    ...
    try{
      ...
      if(d.tracks&&d.tracks.length>0){
        ...
        setError(d.error||"No tracks found from Pixabay")
        setTracks(FALLBACK_TRACKS)
      }
    }catch(e:any){
      setError("Could not reach music API — showing sample tracks")
      setTracks(FALLBACK_TRACKS)
    }
    setLoading(false)
  }
  ...
  function togglePlay(track:any){
    if(playingId===track.id){audioRefs.current[track.id]?.pause();setPlayingId(null)}
      ...
      if(!audioRefs.current[track.id]){const a=new Audio(track.preview_url);audioRefs.current[track.id]=a;a.onended=()=>setPlayingId(null)}
      audioRefs.current[track.id].play().catch(()=>setError("Could not play preview — try another track"))
  ...
  return <div style={{ background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:20 }}>
    ...
    <div style={{ fontSize:13,color:C.muted,marginBottom:16 }}>Optional — choose a royalty-free track to play behind your ad.</div>
    {suggestedMood&&<div style={{ background:"#6c63ff11",border:"1px solid #6c63ff33",borderRadius:8,padding:"8px 12px",fontSize:12,color:C.accent,marginBottom:12 }}>✨ AI suggested: <strong>{suggestedMood}</strong> based on your script type</div>}
    {error&&<div style={{ background:"#f59e0b11",border:"1px solid #f59e0b33",borderRadius:8,padding:"8px 12px",fontSize:11,color:"#fbbf24",marginBottom:12 }}>⚠️ {error}</div>}
    ...
    <div style={{ display:"flex",flexDirection:"column",gap:8,maxHeight:280,overflowY:"auto" }}>
      {tracks.map((track:any)=>(
        <div key={track.id} onClick={()=>setSelectedTrack(selectedTrack?.id===track.id?null:track)} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 12px",borderRadius:10,border:"2px solid "+(selectedTrack?.id===track.id?C.accent:C.border),background:selectedTrack?.id===track.id?C.accentSoft:C.surface,cursor:"pointer" }}>
          ...
          {selectedTrack?.id===track.id&&<span style={{ color:C.accent,fontSize:12,fontWeight:700,flexShrink:0 }}>✓ Selected</span>}
        </div>
      ))}
    </div>
...
function ExportVideo({ sections, libraryItems, voiceoverUrl, musicUrl }: any) {
  ...
  const clips = (sections||[]).map((s:any)=>{
    ...
    return{item,start:item.start_seconds||0,end:item.end_seconds||(item.start_seconds||0)+5}
  }).filter(Boolean)

  async function doExport(){
    if(!clips.length){alert("No clips assigned — assign clips to sections first.");return}
    setExporting(true);setProgress(0);setMsg("Setting up recorder…")
    try{
      ...
      if(voiceoverUrl){
        voiceEl=new Audio(voiceoverUrl)
      ...
      const allTracks=[...videoStream.getVideoTracks(),...dest.stream.getAudioTracks()]
      ...
      for(let i=0;i<clips.length;i++){
        const clip=clips[i]
        setMsg(`Rendering clip ${i+1} of ${clips.length}…`);setProgress(Math.round((i/clips.length)*85))
        await new Promise<void>(resolve=>{
          const vid=document.createElement('video')
          vid.crossOrigin='anonymous';vid.muted=true;vid.playsInline=true
          vid.src=`https://stream.mux.com/${clip.item.mux_playback_id}.m3u8`
          const timeout=setTimeout(()=>{vid.pause();resolve()},15000)
          vid.onloadedmetadata=()=>{
            ...
            vid.onseeked=()=>{
              ...
              let raf:number
              function draw(){
                try{ctx2d.drawImage(vid,0,0,720,1280)}catch(e){}
                if(vid.currentTime>=clip.end||vid.ended){vid.pause();clearTimeout(timeout);cancelAnimationFrame(raf);resolve()}
      ...
      setMsg("Finalising video…");setProgress(90)
      recorder.stop()
      ...
      const url=URL.createObjectURL(blob)
      const a=document.createElement('a');a.href=url;a.download=`adforge-ad-${Date.now()}.webm`;a.click()
      setTimeout(()=>URL.revokeObjectURL(url),10000)
      setMsg("✓ Downloaded!");setProgress(100)
    }catch(e:any){setMsg("Export failed: "+e.message);console.error(e)}
    setExporting(false)
  }
  ...
  return <div style={{ background:C.card,border:"1px solid "+C.border,borderRadius:14,padding:20,marginTop:16 }}>
    <div style={{ fontWeight:700,fontSize:16,marginBottom:4 }}>⬇️ Export Final Ad</div>
    <div style={{ fontSize:13,color:C.muted,marginBottom:12 }}>Compile all clips{voiceoverUrl?" + voiceover":""}{musicUrl?" + background music":""} into one downloadable video.</div>

    <div style={{ display:"flex",gap:12,marginBottom:14,flexWrap:"wrap" }}>
      <div style={{ background:C.surface,border:"1px solid "+C.border,borderRadius:8,padding:"8px 14px",fontSize:12 }}>🎬 {assignedCount}/{totalSections} clips assigned</div>
      {voiceoverUrl&&<div style={{ background:"#22c55e11",border:"1px solid #22c55e33",borderRadius:8,padding:"8px 14px",fontSize:12,color:C.green }}>🎙️ Voiceover ready</div>}
      {musicUrl&&<div style={{ background:"#6c63ff11",border:"1px solid #6c63ff33",borderRadius:8,padding:"8px 14px",fontSize:12,color:C.accent }}>🎵 Music selected</div>}
    </div>
    ...
    {msg&&!exporting&&<div style={{ fontSize:13,color:msg.includes("✓")?C.green:C.red,marginBottom:12,fontWeight:600 }}>{msg}</div>}

    <div style={{ background:"#ffffff08",border:"1px solid "+C.border,borderRadius:8,padding:"8px 12px",fontSize:11,color:C.muted,marginBottom:14 }}>💡 Exports as WebM — compatible with Chrome, Firefox, VLC, Premiere, DaVinci Resolve, and most video editors.</div>
    ...
    <Btn onClick={doExport} disabled={exporting||assignedCount===0} style={{ background:exporting?C.border:C.green,color:exporting?"#aaa":"#000",fontWeight:700,width:"100%",padding:14,fontSize:15,borderRadius:12 }}>
      {exporting?`⏳ ${msg}`:"⬇️ Download Final Ad (WebM)"}
    </Btn>
  </div>
}
...
function ScriptTable({ sections, onChange, libraryItems, readOnly, brandName, productName, voiceoverUrl }: any) {
  ...
  return <div>
    ...
    <div style={{ display:"flex",alignItems:"center",gap:12,padding:"10px 16px",background:C.surface,borderBottom:"1px solid "+C.border }}>
      ...
      {voiceoverUrl&&<div style={{ fontSize:11,color:C.green }}>🎙️ Voiceover active — click ▶ on clips to preview with audio context</div>}
      <div style={{ flex:1 }}/>
      <button onClick={toggleMuteAll} style={{ background:allMuted?"#ef444422":C.accentSoft,border:"1px solid "+(allMuted?"#ef444466":C.accent+"44"),color:allMuted?"#ef4444":C.accent,borderRadius:8,padding:"5px 12px",cursor:"pointer",fontSize:12,fontWeight:600 }}>{allMuted?"🔇 All Muted":"🔊 Mute All Clips"}</button>
    </div>
...
function ScriptsTab({ scripts, items, brand, products, onSaveScripts, onSaveForgedAd }: any) {
  ...
  // Step nav — new order: Voiceover → Script & Clips → Music → Preview
  const reviewSteps=[
    {id:"voiceover",label:"1. Voiceover"},
    {id:"script",label:"2. Script & Clips"},
    {id:"music",label:"3. Music"},
  ...
  if(view==="list")return<div style={{ maxWidth:820,margin:"0 auto",padding:28 }}>
    <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24 }}>
      ...
      <Btn onClick={()=>{setView("generate");setSections([]);setGenMeta(null);setVoiceoverUrl(null);setMusicUrl(null)}} style={{ background:C.accent,color:"#fff" }}>+ New Script</Btn>
    </div>
    {scripts.length===0?<Card style={{ textAlign:"center",padding:60 }}><div style={{ fontSize:40,marginBottom:12 }}>✍️</div><STitle mb={6}>No scripts yet</STitle><Btn onClick={()=>setView("generate")} style={{ background:C.accent,color:"#fff",marginTop:8 }}>Generate First Script</Btn></Card>
  ...
  if(view==="review"){
    ...
    return<div style={{ padding:20 }}>
      ...
      {step==="voiceover"&&<div style={{ maxWidth:640,margin:"0 auto" }}>
        ...
        <div style={{ background:"#6c63ff11",border:"1px solid #6c63ff33",borderRadius:10,padding:"10px 14px",fontSize:13,color:C.accent,marginBottom:16 }}>💡 Generate your voiceover first — then you can select clips while listening to it for context.</div>
        <VoiceoverGenerator sections={sections} savedVoiceoverUrl={voiceoverUrl} onSave={(url:string|null,voice:string|null)=>{setVoiceoverUrl(url);setVoiceoverVoice(voice);setStep("script")}}/>
      </div>}
      ...
      {step==="script"&&<>
        ...
        {voiceoverUrl&&<div style={{ background:"#22c55e11",border:"1px solid #22c55e33",borderRadius:10,padding:"10px 16px",marginBottom:12,fontSize:13,color:"#4ade80",display:"flex",alignItems:"center",gap:10 }}>🎙️ Voiceover active — <audio src={voiceoverUrl} controls style={{ height:28,flex:1 }}/></div>}
        {autoCount>0&&<div style={{ background:"#22c55e11",border:"1px solid #22c55e33",borderRadius:10,padding:"10px 16px",marginBottom:14,fontSize:13,color:"#4ade80" }}>✦ AI auto-selected {autoCount} clip{autoCount!==1?"s":""} — swap any using the timeline below.</div>}
      ...
      {step==="music"&&<div style={{ maxWidth:640,margin:"0 auto" }}>
        ...
        <MusicPicker suggestedMood={suggestedMood} savedMusicUrl={musicUrl} savedMusicName={musicName} onSave={(url:string|null,name:string|null)=>{setMusicUrl(url);setMusicName(name);setStep("preview")}}/>
      </div>}
      ...
      {step==="preview"&&<div style={{ maxWidth:860,margin:"0 auto" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20,flexWrap:"wrap",gap:10 }}>
          ...
          <div style={{ display:"flex",gap:10 }}>
            ...
            <Btn onClick={()=>handleSaveForged("complete")} style={{ background:C.green,color:"#000",fontWeight:700 }}>✓ Mark as Complete</Btn>
          </div>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20 }}>
          <Card>
            <div style={{ fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:8 }}>🎙️ Voiceover</div>
            {voiceoverUrl?<><audio src={voiceoverUrl} controls style={{ width:"100%",height:36,marginBottom:6 }}/><div style={{ fontSize:12,color:C.green }}>✓ {voiceoverVoice}</div><button onClick={()=>setStep("voiceover")} style={{ background:"none",border:"none",color:C.muted,fontSize:11,cursor:"pointer",textDecoration:"underline",padding:0,marginTop:4 }}>Change</button></>:<div style={{ fontSize:13,color:C.muted }}>No voiceover — <button onClick={()=>setStep("voiceover")} style={{ background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:13,textDecoration:"underline",padding:0 }}>add one</button></div>}
          </Card>
          <Card>
            <div style={{ fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:8 }}>🎵 Music</div>
            {musicUrl?<><audio src={musicUrl} controls style={{ width:"100%",height:36,marginBottom:6 }}/><div style={{ fontSize:12,color:C.green }}>✓ {musicName}</div><button onClick={()=>setStep("music")} style={{ background:"none",border:"none",color:C.muted,fontSize:11,cursor:"pointer",textDecoration:"underline",padding:0,marginTop:4 }}>Change</button></>:<div style={{ fontSize:13,color:C.muted }}>No music — <button onClick={()=>setStep("music")} style={{ background:"none",border:"none",color:C.accent,cursor:"pointer",fontSize:13,textDecoration:"underline",padding:0 }}>add some</button></div>}
          </Card>
        </div>
        <StitchedPreview sections={sections} libraryItems={items}/>
  ...
  return<div style={{ maxWidth:900,margin:"0 auto",padding:28 }}>
    ...
    {previewAd&&<div onClick={()=>setPreviewId(null)} style={{ position:"fixed",inset:0,background:"#000000ee",zIndex:300,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:20,overflowY:"auto" }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:C.surface,border:"1px solid "+C.border,borderRadius:16,padding:24,maxWidth:900,width:"100%",marginTop:20 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
          <div><div style={{ fontWeight:700,fontSize:18 }}>{previewAd.title}</div><div style={{ fontSize:13,color:C.muted }}>Created {previewAd.created_at?new Date(previewAd.created_at).toLocaleDateString():""}</div></div>
    ...
    {filtered.length===0?<Card style={{ textAlign:"center",padding:60 }}><div style={{ fontSize:40,marginBottom:12 }}>⚡</div><STitle mb={6}>{statusFilter==="draft"?"No drafts yet":statusFilter==="complete"?"No completed ads yet":"No forged ads yet"}</STitle><div style={{ color:C.muted,fontSize:13 }}>Generate a script and save it as a draft or completed ad.</div></Card>
    :<div style={{ display:"grid",gap:14 }}>{filtered.map(ad=>(
      <Card key={ad.id}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12 }}>
          <div style={{ flex:1,minWidth:0 }}>
            <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:6,flexWrap:"wrap" }}>
              ...
              {ad.voiceover_url&&<span style={{ fontSize:10,color:C.green }}>🎙️ Voiceover</span>}
              {ad.music_url&&<span style={{ fontSize:10,color:C.accent }}>🎵 Music</span>}
            </div>
        ...
        {ad.sections&&ad.sections.length>0&&(
          <div style={{ display:"flex",gap:6,overflowX:"auto",paddingBottom:4 }}>
            {ad.sections.map((s:any,i:number)=>{
              ...
              return<div key={i} style={{ flexShrink:0,textAlign:"center" }}>
                <div style={{ width:44,height:78,background:"#111",borderRadius:6,overflow:"hidden",border:"1px solid "+C.border,position:"relative" }}>
                  {item?.mux_playback_id?<img src={muxThumb(item.mux_playback_id,item.thumbnail_time||0)} alt="" style={{ width:"100%",height:"100%",objectFit:"cover" }}/>:<div style={{ width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16 }}>🎬</div>}
                </div>
                <div style={{ fontSize:7,fontWeight:800,color:sc.color,marginTop:2 }}>{s.type}</div>
              </div>
            })}
          </div>
        )}
  ...
  return<div style={{ maxWidth:820,margin:"0 auto",padding:28 }}>
    ...
    {section==="brand"&&<Card>
      <div style={{ marginBottom:16 }}>
        ...
        <div style={{ fontSize:11,color:C.muted,marginTop:6 }}>AI visits your website and fills fields in first-person brand voice. You can edit anything afterwards.</div>
      </div>
    ...
    {section==="avatars"&&<div>
      ...
      {(brand.customer_avatars||[]).length===0&&!editingAvatar?<Card style={{ textAlign:"center",padding:60 }}><div style={{ fontSize:40,marginBottom:12 }}>👤</div><STitle mb={6}>No avatars yet</STitle><div style={{ color:C.muted,fontSize:13 }}>Customer avatars help generate more targeted scripts.</div></Card>
