...
async function callClaude(messages: any[], maxTokens = 1500) {
  ...
  if(!res.ok){
    ...
    if(typeof msg==="string"&&msg.toLowerCase().includes("credit"))throw new Error("Your Anthropic API credits are empty. Go to console.anthropic.com → Billing to top up.")
    throw new Error(msg)
  }
  if(!d.text)throw new Error("Claude returned an empty response — try again.")
  return d.text
}
...
function ScriptTable({sections,onChange,libraryItems,readOnly,brandName,productName,brandVoice,voiceoverUrl}:any){
  ...
  async function autofillRow(idx:number){
    ...
    try{const ctx=sections.map((s:any,i:number)=>`[${i===idx?"→ THIS":"  "}] ${s.type}: ${(s.spokenWords||"(empty)").substring(0,60)}`).join("\n");const raw=await callClaude([{role:"user",content:`Write a ${row.type} section for a direct response video ad.\nBrand: ${brandName||"Unknown"}\nProduct: ${productName||"Unknown"}\nBrand voice/tone: ${brandVoice||"persuasive and direct"}\n\nScript context:\n${ctx}\n\nIMPORTANT: Match the brand voice exactly. Keep it punchy and direct-response.\nReturn ONLY JSON: {"spokenWords":"exact words spoken","visualDirection":"what is on screen"}`}],500);const data=JSON.parse(raw.replace(/```json|```/g,"").trim());updM(idx,{spokenWords:data.spokenWords||row.spokenWords,visualDirection:data.visualDirection||row.visualDirection})}catch(e){console.error(e)}
    setFillingIdx(null)
  }

  return<div>
    {pickerIdx!==null&&<ClipPickerModal currentId={sections[pickerIdx]?.selectedClipId} matchedIds={sections[pickerIdx]?.matchedClipIds||[]} matchReason={sections[pickerIdx]?.matchReason||""} libraryItems={libraryItems} sectionLabel={sections[pickerIdx]?.type||""} onSelect={(id:string)=>updM(pickerIdx,{selectedClipId:id,autoSelected:false})} onClose={()=>setPickerIdx(null)}/>}
    <div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",background:C.surface,borderBottom:"1px solid "+C.border}}>
      ...
      {(()=>{const total=sections.reduce((acc:number,s:any)=>{const m=(s.durationEstimate||"").match(/(\d+)/);return acc+(m?parseInt(m[1]):0)},0);return total>0?<span style={{fontSize:12,color:C.accent,fontWeight:600}}>~{total}s estimated</span>:null})()}
...
function ClipPickerModal({currentId,matchedIds,matchReason,libraryItems,sectionLabel,onSelect,onClose}:any){
  ...
  return<div onClick={onClose} style={{position:"fixed",inset:0,background:"#000000dd",zIndex:300,display:"flex",alignItems:"flex-start",justifyContent:"center",padding:20,overflowY:"auto"}}>
    <div onClick={e=>e.stopPropagation()} style={{background:C.surface,border:"1px solid "+C.border,borderRadius:16,padding:24,maxWidth:760,width:"100%",marginTop:40}}>
      ...
      {matchReason&&<div style={{background:"#22c55e11",border:"1px solid #22c55e33",borderRadius:8,padding:"8px 12px",fontSize:12,color:C.green,marginBottom:14}}>🎯 AI matched on: {matchReason}</div>}
...
// ── Scripts Tab ───────────────────────────────────────────────────────────
function ScriptsTab({scripts,items,brand,products,forgedAds,onSaveScripts,onSaveForgedAd,onGoToForged}:any){
  ...
  useEffect(()=>{
    try{
      ...
      if(saved){const d=JSON.parse(saved);if(d.sections?.length>0){setSections(d.sections);setGenMeta(d.genMeta||null);setVoiceoverUrl(d.voiceoverUrl||null);setVoiceoverVoice(d.voiceoverVoice||null);setMusicUrl(d.musicUrl||null);setMusicName(d.musicName||null);setSuggestedMood(d.suggestedMood||"Uplifting");if(d.form)setForm(d.form);setView("review");setStep(d.step||"voiceover")}}
    }catch(e){}
  },[])
  useEffect(()=>{
    if(sections.length>0&&view==="review"){
      try{localStorage.setItem("adforge_draft",JSON.stringify({sections,genMeta,voiceoverUrl,voiceoverVoice,musicUrl,musicName,suggestedMood,form,step}))}catch(e){}
    }
  ...
  async function matchClips(secs:any[],libItems:Item[]){
    const sectionHints:Record<string,string>={
      "HOOK":"high-energy, pattern interrupt, punchy visual, under 5s, grabs attention immediately",
      "PROBLEM":"relatable struggle, emotional pain point, before state, problem demonstration",
      "AGITATE":"amplifying the problem, contrast, frustration, before/after",
      "SOLUTION":"product demo, clear benefit, transformation, product close-up",
      "SOCIAL PROOF":"testimonial, talking head, user reaction, review, real person",
      "CTA":"urgency, product shot, discount, call to action, buy now energy",
      "BODY":"general lifestyle, product use, supporting visuals"
    }
    const libSummary=libItems.map(item=>`ID:${item.id}|title:${item.title}|creator:${item.creator||""}|tags:${(item.analysis?.scene_tags||[]).join(",")}|use:${item.analysis?.use_case||""}|type:${item.analysis?.content_type||""}`).join("\n")
    const scriptDesc=secs.map((s:any,i:number)=>`Section ${i} [${s.type}] (ideal clip: ${sectionHints[s.type]||"general"}): spoken="${(s.spokenWords||"").substring(0,80)}"`).join("\n")
    const prompt=`Match script sections to video clips. Prioritise clips whose tags and type match the section's ideal clip description.\nSCRIPT:\n${scriptDesc}\nLIBRARY:\n${libSummary}\nReturn ONLY valid JSON array: [{"section":0,"clip_ids":["uuid1"],"best_id":"uuid1","reason":"why this clip matches"},…]`
    try{
      ...
      return secs.map((s:any,i:number)=>{const m=matches.find((x:any)=>x.section===i);const bestId=m?.best_id&&validIds.has(m.best_id)?m.best_id:null;const matchedIds=(m?.clip_ids||[]).filter((id:string)=>validIds.has(id));return{...s,matchedClipIds:matchedIds.length>0?matchedIds:(s.matchedClipIds||[]),selectedClipId:bestId||(s.selectedClipId||null),autoSelected:!!bestId,matchReason:m?.reason||""}})
    }catch(e){return secs}
  }

  async function handleGenVariants(){
    setGeneratingVariants(true);setGenError("")
    try{
      const prod=products.find((x:Product)=>String((x as any).id)===String(form.productId))||null
      const stage=STAGES.find(s=>s.value===form.awarenessStage)||STAGES[0]
      let ctx=`BRAND:\nName: ${brand.name||"Unknown"}\nDesc: ${brand.description||""}\nVoice: ${brand.voice||""}\nCustomer: ${brand.target_customer||""}\n\n`
      if(prod)ctx+=`PRODUCT:\nName: ${prod.name}\nDesc: ${prod.description||""}\nBenefits: ${prod.benefits||""}\nClaims: ${prod.claims||""}\n\n`
      const angles=["UGC","Testimonial","Problem-Solution"]
      const results=await Promise.all(angles.map(async(angle)=>{
        const prompt=ctx+`SCRIPT REQ:\nContent type: ${angle}\nLength: ${form.adLength}\nStage: ${stage.label} — ${stage.desc}\nCustomer: ${form.customerAvatar||brand.target_customer||""}\nPains: ${form.painPoints||""}\nRequest: ${form.request||""}\n\nWrite a direct response video ad script with a unique angle. Return ONLY valid JSON:\n{"sections":[{"id":1,"type":"HOOK","spokenWords":"exact words","visualDirection":"what is on screen","durationEstimate":"0-3s"}],"suggested_music_mood":"Uplifting","angle_label":"one line describing this angle's hook approach"}`
        try{
          const raw=await callClaude([{role:"user",content:prompt}],2000)
          const data=JSON.parse(raw.replace(/```json|```/g,"").trim())
          let secs=(data.sections||[]).map((s:any,i:number)=>({...s,id:Date.now()+i+Math.random(),matchedClipIds:[],selectedClipId:null,autoSelected:false}))
          if(items.length>0)secs=await matchClips(secs,items)
          return{contentType:angle,sections:secs,mood:data.suggested_music_mood||"Uplifting",label:data.angle_label||angle}
        }catch(e){return null}
      }))
      setVariants(results.filter(Boolean))
    }catch(e:any){setGenError(e.message||"Error generating variants.")}
 ...
 async function handleSaveForged(status:"draft"|"complete"){
  ...
  const basePattern=`${stageWords}_${form.contentType||"UGC"}_${(form.adLength||"30 seconds").replace(" seconds","s")}`
  // Auto-increment version
  const existingVersions=(forgedAds||[]).filter((a:ForgedAd)=>(a.title||"").startsWith(basePattern)).map((a:ForgedAd)=>{const m=(a.title||"").match(/_v(\d+)$/);return m?parseInt(m[1]):1})
  const nextVersion=existingVersions.length>0?Math.max(...existingVersions)+1:1
  const autoName=`${basePattern}_v${nextVersion}`
  ...
  if(view==="list")return<div style={{maxWidth:820,margin:"0 auto",padding:28}}>
    ...
    {sections.length>0&&<div style={{background:"#6c63ff11",border:"1px solid #6c63ff33",borderRadius:10,padding:"12px 16px",marginBottom:20,display:"flex",alignItems:"center",gap:12}}>
      ...
      <Btn onClick={()=>{setView("review");setStep("voiceover")}} style={{background:C.accent,color:"#fff",fontSize:12,padding:"7px 14px"}}>Resume →</Btn>
      <Btn onClick={()=>{setSections([]);clearDraft()}} style={{background:"none",border:"1px solid "+C.border,color:C.muted,fontSize:12,padding:"7px 14px"}}>Discard</Btn>
    </div>}
  ...
  if(view==="generate")return<div style={{maxWidth:740,margin:"0 auto",padding:28}}>
    ...
    <Btn onClick={handleGen} disabled={generating} style={{background:generating?C.border:C.accent,color:"#fff",width:"100%",padding:14,fontSize:16,borderRadius:12,marginBottom:10}}>{generating?`⏳ ${genStatus||"Generating…"}`:"✨ Generate Script"}</Btn>
    <Btn onClick={handleGenVariants} disabled={generatingVariants||generating} style={{background:generatingVariants?C.border:"transparent",color:generatingVariants?C.muted:C.accent,border:"1px solid "+C.accent+"55",width:"100%",padding:12,fontSize:14,borderRadius:12}}>{generatingVariants?"⏳ Generating 3 angles…":"🎲 Generate 3 Angles — pick the best"}</Btn>
    {genError&&<div style={{background:"#ef444422",border:"1px solid #ef444433",borderRadius:10,padding:"12px 16px",fontSize:13,color:"#ef4444",marginTop:12}}>⚠️ {genError}</div>}
    {variants.length>0&&<div style={{marginTop:24}}>
      <div style={{fontWeight:700,fontSize:16,marginBottom:4,color:C.text}}>Pick an angle</div>
      <div style={{fontSize:13,color:C.muted,marginBottom:16}}>3 different approaches generated — click one to use it.</div>
      <div style={{display:"grid",gap:14}}>
        {variants.map((v:any,i:number)=>{
          const hook=v.sections?.find((s:any)=>s.type==="HOOK")||v.sections?.[0]
          return<div key={i} onClick={()=>{setSections(v.sections);setGenMeta({form:{...form,contentType:v.contentType},productName:products.find((x:Product)=>String((x as any).id)===String(form.productId))?.name||"General"});setSuggestedMood(v.mood);setVariants([]);setView("review");setStep("voiceover")}} style={{background:C.card,border:"2px solid "+C.border,borderRadius:14,padding:18,cursor:"pointer",transition:"border-color 0.15s"}} onMouseOver={e=>(e.currentTarget as any).style.borderColor=C.accent} onMouseOut={e=>(e.currentTarget as any).style.borderColor=C.border}>
            <div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}>
              <span style={{background:C.accentSoft,color:C.accent,border:"1px solid "+C.accent+"44",borderRadius:99,fontSize:10,fontWeight:700,padding:"2px 8px"}}>{v.contentType}</span>
              <span style={{fontSize:12,color:C.muted}}>{v.sections?.length} sections · {v.mood} music</span>
            </div>
            <div style={{fontWeight:600,fontSize:13,color:C.text,marginBottom:6}}>{v.label}</div>
            {hook&&<div style={{fontSize:13,color:C.muted,fontStyle:"italic",overflow:"hidden",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical" as any}}>"{hook.spokenWords}"</div>}
          </div>
        })}
      </div>
    </div>}
  ...
  if(view==="review"){
    ...
    return<div style={{padding:20}}>
      ...
      {step==="voiceover"&&<div style={{maxWidth:640,margin:"0 auto"}}>
        ...
        {genError&&<div style={{background:"#ef444422",border:"1px solid #ef444433",borderRadius:10,padding:"12px 16px",fontSize:13,color:"#ef4444",marginBottom:16}}>⚠️ {genError}</div>}
      ...
      {step==="script"&&<>
        ...
        <Card style={{padding:0,overflow:"hidden",marginBottom:20}}>
          <ScriptTable sections={sections} onChange={setSections} libraryItems={items} readOnly={false} brandName={brand.name} productName={genMeta?.productName} brandVoice={brand.voice} voiceoverUrl={voiceoverUrl}/>
        </Card>
  ...
  if(view==="detail"&&selected){
    ...
    return<div style={{padding:28}}>
      ...
      <Card style={{padding:0,overflow:"hidden",marginBottom:20}}>
        <ScriptTable sections={disp} onChange={setSections} libraryItems={items} readOnly={false} brandName={brand.name} productName={selected.product_name} brandVoice={brand.voice}/>
      </Card>
...
export default function AdForgeApp(){
  ...
  return<div style={{background:C.bg,minHeight:"100vh",fontFamily:"'Inter',system-ui,sans-serif",color:C.text}}>
    ...
    {tab==="scripts"&&<ScriptsTab scripts={scripts} items={items} brand={brand} products={products} forgedAds={forgedAds} onSaveScripts={setScripts} onSaveForgedAd={handleSaveForgedAd} onGoToForged={()=>setTab("forged")}/>}
